...
import { careRouter } from './care/router.js';
...
/** Build the HTTP app. Separate from listening so tests can mount it directly. */
export function createApp(vault: VaultClient) {
  const app = express();
  ...
  // The care network parses its own (smaller) JSON bodies.
  app.use('/care', careRouter(vault));
